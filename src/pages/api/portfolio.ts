...
async function fetchStockData(stock: stockDataType) {
  const trySuffixes = [".NS", ".BO"];

  for (const suffix of trySuffixes) {
    try {
      const yahoo = await yahooFinance.quoteSummary(
      ...
      const eps = yahoo.defaultKeyStatistics?.trailingEps;
  ...
  console.error(
    `Yahoo data fetch failed for ${stock.name} (${stock.exchange}) on both NSE and BSE`
  );
